...
export interface BrandSignature {
  ...
  width: number;
  height: number;
  grayscaleBuffer: Uint8ClampedArray;
}
...
export class AiEngineCore {
  ...
  private brandSignatures: Map<string, BrandSignature> = new Map();
  private confusableMap: Map<string, string> = new Map();

  // Top high-value target brand domains monitored globally
  private readonly TOP_TARGET_BRANDS: Record<string, string[]> = {
   ...
   */
  public async runTextInference(content: string): Promise<InferenceResult> {
   ...
   */
  public matchVisualBrandImpersonation(
    frameGrayscale: Uint8ClampedArray,
    width: number,
    height: number,
    currentOriginDomain: string
  ): { impersonating: boolean; matchedBrand: string | null; ssimScore: number } {
    let highestScore = 0;
    ...
    for (const [brandName, signature] of this.brandSignatures.entries()) {
      ...
      const isLegitimate = signature.officialDomains.some(
        (legit) => currentOriginDomain === legit || currentOriginDomain.endsWith(`.${legit}`)
      );
   ...
   */
  public evaluateDomainHomograph(domain: string): HomographCheckResult {
    ...
    for (const char of cleanDomain) {
      ...
      if (code <= 0x7f) {
        ...
        hasNonAscii = true;
        if (code >= 0x0400 && code <= 0x04ff) detectedScripts.add("Cyrillic");
        else if (code >= 0x0370 && code <= 0x03ff) detectedScripts.add("Greek");
        else if (code >= 0x0590 && code <= 0x05ff) detectedScripts.add("Hebrew");
        else if (code >= 0x0600 && code <= 0x06ff) detectedScripts.add("Arabic");
        else detectedScripts.add("Unicode-Other");
      }
    }
   ...
   */
  public computeLevenshtein(a: string, b: string): number {
    const matrix: number[][] = [];
   ...
   */
  private sanitizeAdversarialInput(input: string): string {
   ...
   */
  private evaluatePerturbationEntropy(original: string, sanitized: string): number {
   ...
   */
  private initConfusableMap(): void {
   ...
   */
  private seedDefaultBrandSignatures(): void {
