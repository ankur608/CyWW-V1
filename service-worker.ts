...
import {
  ThreatMapper,
  ThreatTelemetrySignal,
  SecurityIncident,
  ZtaAttestationTokenPayload
} from "./threat-mapper.js";
...
interface MessageRequest {
  ...
  domain?: string;
  url?: string;
  frameBuffer?: number[]; // Grayscale pixel array
  width?: number;
  height?: number;
...
class ServiceWorkerOrchestrator {
  ...
  private secretHmacKey: string = "cyww-defense-hmac-master-key-prod-zta";
  private activeSessions: Map<string, SessionState> = new Map();
  private dynamicRuleCounter: number = 1000;
  ...
  private registerLifecycles(): void {
  ...
  private async initStorage(): Promise<void> {
    ...
    if (!existingKey.cyww_hmac_key) {
      ...
      this.secretHmacKey = existingKey.cyww_hmac_key as string;
    }
  }
   ...
   */
  private async handleTabNavigation(tabId: number, urlString: string): Promise<void> {
    ...
    try {
      ...
      const signals: ThreatTelemetrySignal[] = [];
      ...
      if (isQuarantined) {
        ...
        const token = await ThreatMapper.generateAttestationToken(
          domain,
          `TAB-${tabId}`,
          100 - riskScore,
          homographResult.targetBrandMatch,
          this.secretHmacKey
        );
    ...
    switch (request.action) {
      ...
      case "VERIFY_INTERACTION": {
        ...
        if (request.formDetails?.action) {
          try {
            const actionDomain = new URL(
              request.formDetails.action,
              sender.tab?.url
            ).hostname;
      ...
      case "ANALYZE_CANVAS_FRAME": {
        ...
        const match = this.aiEngine.matchVisualBrandImpersonation(
          buffer,
          request.width,
          request.height,
          origin
      ...
      case "GET_ALL_INCIDENTS": {
        ...
        return { incidents: (data.cyww_incident_ledger as SecurityIncident[]) || [] };
      }
   ...
   */
  private async inspectDownloadItem(downloadItem: chrome.downloads.DownloadItem): Promise<void> {
    try {
   ...
   */
  private verifyMagicBytes(bytes: Uint8Array, mime: string, filename: string): boolean {
    ...
    if (
      (mime.includes("image") || lowerFilename.endsWith(".png") || lowerFilename.endsWith(".jpg")) &&
      isPE
    ) {
   ...
   */
  private calculateShannonEntropy(data: Uint8Array): number {
   ...
   */
  private async applyDynamicDnrBlock(domain: string): Promise<void> {
  ...
  private async recordSecurityIncident(
    domain: string,
    risk: number,
    signals: ThreatTelemetrySignal[],
    verdict: SecurityIncident["verdict"]
  ): Promise<void> {
    ...
    const ledger = (existing.cyww_incident_ledger as SecurityIncident[]) || [];
    ledger.push(incident);
    await chrome.storage.local.set({ cyww_incident_ledger: ledger });
  }

  private setupDnrRules(): void {
