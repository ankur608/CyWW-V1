...
interface ZtaVerificationResponse {
  allowed: boolean;
...
interface BrandImpersonationResponse {
...
class ContentScriptGuardian {
  private isOriginVerified: boolean = false;
  private currentAttestationToken: string | null = null;
  private mutationObserver: MutationObserver | null = null;
  private lastCapturedFrameTime: number = 0;
   ...
   */
  private async verifyCurrentOrigin(): Promise<void> {
    try {
      const response = (await chrome.runtime.sendMessage({
        action: "VERIFY_ORIGIN",
        domain: window.location.hostname,
        url: window.location.href
      })) as ZtaVerificationResponse;
   ...
   */
  private hookUserInteractions(): void {
        ...
        void chrome.runtime
          .sendMessage({
            ...
            formDetails: {
              ...
              hasPassword,
              inputNames: form
                ? Array.from(form.elements).map((el) => (el as HTMLInputElement).name || "")
                : [(target as HTMLInputElement).name || ""]
            }
          })
          .then((res: ZtaVerificationResponse) => {
            if (!res.allowed) {
              this.lockdownInput(target as HTMLInputElement, res.reason || "Interaction Blocked");
            } else {
              this.isOriginVerified = true;
          ...
          for (let i = 0; i < mutation.addedNodes.length; i++) {
            ...
            if (node.nodeType === Node.ELEMENT_NODE) {
              ...
              if (el.tagName === "IFRAME") {
                this.inspectIframe(el as HTMLIFrameElement);
              }
   ...
   */
  private detectUiRedressing(): void {
   ...
   */
  private inspectIframe(iframe: HTMLIFrameElement): void {
   ...
   */
  private async performVisualSpatialScan(): Promise<void> {
    const now = Date.now();
    if (now - this.lastCapturedFrameTime < 5000) return; // Throttle visual checks
    ...
    try {
      ...
      const response = (await chrome.runtime.sendMessage({
        ...
        width: targetWidth,
        height: targetHeight
      })) as BrandImpersonationResponse;

      if (response && response.impersonating) {
        this.lockdownPage(
          `Visual Brand Impersonation Flagged: High structural similarity to authenticated '${response.matchedBrand}' portal.`
        );
      }
    } catch (err) {
   ...
   */
  private lockdownInput(input: HTMLInputElement, message: string): void {
    input.disabled = true;
   ...
   */
  private neutralizeMaliciousElement(el: Element, reason: string): void {
   ...
   */
  private lockdownPage(alertReason: string): void {
